	...
	if (action == "roll")
		...
		if(ishuman(usr))
			...
			if(new_turf.x > SSatlas.current_map.overmap_size || new_turf.y > SSatlas.current_map.overmap_size)
				...
				return
			if(do_after(H, 1 SECOND) && connected.can_combat_roll())
				visible_message(SPAN_DANGER("[H] remotely tilts the yoke systematically to the [ndir == WEST ? "left" : "right"]!"))
				connected.combat_roll(ndir)
	 ...
	 // AI and robots are allowed to pilot now
		...
		if (action == "combat_turn")
			...
			if(issilicon(usr))
				var/mob/living/carbon/human/H = usr
				if(do_after(H, 1 SECOND) && connected.can_combat_turn())
					visible_message(SPAN_DANGER("[H] remotely twists the yoke systematically to the [ndir == WEST ? "left" : "right"]!"))
					connected.combat_turn(ndir)
